  ...
  <div class="card">
    ...
    <div class="gauges-container">
      ...
      <div class="gauge-wrapper">
        ...
        <div class="gauge-value" id="pwmGaugeValue">— PWM</div>
      </div>
    </div>
  </div>
...
<script>
  (function(){
    ...
    const pwmGauge  = new Gauge('pwmGauge',  0,   255, 'PWM', '#FFA07A');
      ...
      try {
        ...
        ws.addEventListener('message', (evt) => {
          try {
            ...
            if (d.pwm !== undefined) {
              const pwmValue = parseFloat(d.pwm);
              if (Number.isFinite(pwmValue)) {
                pwmGauge.draw(pwmValue);
                document.getElementById('pwmGaugeValue').textContent = pwmValue.toFixed(0) + ' PWM';
                pushBounded(hist.pwm, pwmValue, MAX_POINTS);
                record.pwm = pwmValue;  // Guardamos como pwm para mantener compatibilidad con CSV
              }
            }
