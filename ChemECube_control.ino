...
// ===== NUEVO SISTEMA DE PANTALLAS =====
enum OledScreen {
  SCREEN_MAIN,        // Pestaña 1
  SCREEN_MODE_MANUAL, // Pestaña 1.1
  SCREEN_MODE_AUTO    // Pestaña 1.2
};

OledScreen oledScreen = SCREEN_MAIN;
int mainMenuSelection = 0; // 0 = manual, 1 = auto
...
void updateOLED() {
  ...
  switch (oledScreen) {
    ...
    case SCREEN_MODE_MANUAL:
      ...
      display.print("CO2: "); display.println(lastCO2, 0);
      display.print("T: ");   display.println(lastTemp, 1);
      display.print("H: ");   display.println(lastHum, 1);
      display.print("PWM: "); display.println(pwmValue);
      display.println("Rotate encoder to adjust.");
      break;
    ...
    case SCREEN_MODE_AUTO:
      if (warmingUp) {
        ...
        display.print("CO2: "); display.println(lastCO2, 0);
        display.print("T: ");   display.print(lastTemp, 1);
        display.println(" C");
        display.print("H: ");   display.print(lastHum, 1);
        display.println(" %");
        display.print("PWM: "); display.println(pwmValue);
      }
      break;
...
void handleButton() {
  ...
  if (reading == LOW && lastButtonState == HIGH &&
    ...
    if (oledScreen == SCREEN_MAIN) {
      ...
      // salir de manual/auto y volver al menú
...
void loop() {
  ...
  // === CONTROL DE PANTALLAS / MENÚ ===

  // limitar encoder en menú principal
  if (oledScreen == SCREEN_MAIN) {
    if (encoderValue < 0) encoderValue = 0;
    if (encoderValue > 1) encoderValue = 1;
    mainMenuSelection = encoderValue;
  }

  // Lectura del botón del encoder (cambio de pantallas)
  bool reading = digitalRead(ENCODER_SW);
  if (reading == LOW && lastButtonState == HIGH &&
      (millis() - lastDebounceTime) > debounceDelay) {

    if (oledScreen == SCREEN_MAIN) {
      // entrar en modo seleccionado
      oledScreen = (mainMenuSelection == 0) ? SCREEN_MODE_MANUAL : SCREEN_MODE_AUTO;
      if (oledScreen == SCREEN_MODE_AUTO) {
        warmingUp = true;
        warmupStart = millis();
      ...
      // salir y regresar a menú principal
      oledScreen = SCREEN_MAIN;
    }
    lastDebounceTime = millis();
  }
  ...
  // Modo automático: warming up y luego PWM a 255
  if (oledScreen == SCREEN_MODE_AUTO && warmingUp) {
